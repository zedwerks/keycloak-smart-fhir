terraform {
  backend "s3" {
    ...
    region         = var.do_spaces_region  # Set your region (nyc3, sfo3, etc.)
    endpoint       = var.do_spaces_endpoint  # DO endpoint for your region, provided in env variables
    access_key     = var.do_spaces_access_key  # Provided through environment variables
    secret_key     = var.do_spaces_secret_key  # Provided through environment variables
